...
function testnumeric(textfield)
{
   ...
   if (lect != textfield.value.length) {
      alert ("<?php print_string('onlynumerics', 'block_courseshop') ?>");
      textfield.value = "";
      return false;
   }
...
function testidentifier(textfield, minchars)
{
   ...
   if (textfield.value.length < minchars) {
      alert ("<?php print_string('mincharserror', 'block_courseshop') ?>\n " + minchars);
      textfield.value = "";
      textfield.focus();
      return false;
   }
   for (i=0 ; i < textfield.value.length ; i++) {
      if ( ((textfield.value.charAt(i) < 'a') || (textfield.value.charAt(i) > 'z')) && ((textfield.value.charAt(i) < 'A') || !(textfield.value.charAt(i) > 'Z')) && ((textfield.value.charAt(i) < '0') || (textfield.value.charAt(i) > '9')) && textfield.value.charAt(i) != 'z') {
...
function testpassword(textfield, minchars)
{
   ...
   if (textfield.name.substring(textfield.name.length - 1) == '2') {
      if (textfield.form.elements[pass].value != textfield.form.elements[echo].value) {
         alert ("<?php print_string('passwordconfirmerror', 'block_courseshop') ?>");
   ...
   if (textfield.value.length < minchars){
      alert ("<?php print_string('mincharserror', 'block_courseshop') ?>" + minchars);
      textfield.value = "";
      textfield.focus();
      return false;
   }
...
function setupper(textfield)
{
   ...
   for (var i = 0; i < textfield.value.length; i++){
      switch (textfield.value.charAt(i)){
         case 'é' : fieldvalue += "e"; break ;
         case 'è' : fieldvalue += "e"; break ;
         case 'ê' : fieldvalue += "e"; break ;
		   case 'à' : fieldvalue += "a"; break ;
		   case 'ü' : fieldvalue += "u"; break ;
		   case 'ù' : fieldvalue += "u"; break ;
		   case 'î' : fieldvalue += "i"; break ;
		   case 'ï' : fieldvalue += "i"; break ;
		   case 'û' : fieldvalue += "u"; break ;
		   case 'ô' : fieldvalue += "o"; break ;
		   default: fieldvalue += textfield.value.charAt(i);
		}
	}
...
function capitalizewords(textfield)
{
   ...
   for(i = 0 ; i < textfield.value.length; i++) {
	   switch (cap_state) {
		   case 0 : { // not word char
			   if ((textfield.value.charAt(i) >= 'a' && textfield.value.charAt(i) <= 'z') || (textfield.value.charAt(i) >= 'A' && textfield.value.charAt(i) <= 'Z') || (textfield.value.charAt(i) >= '0' && textfield.value.charAt(i) <= '9')){
   				cap_state = 1;
		...
		case 1 : // first word char
			{
			if ((textfield.value.charAt(i) >= 'a' && textfield.value.charAt(i) <= 'z') || (textfield.value.charAt(i) >= 'A' && textfield.value.charAt(i) <= 'Z') || (textfield.value.charAt(i) >= '0' && textfield.value.charAt(i) <= '9'))
				{
				fieldvalue += textfield.value.charAt(i).toLowerCase();
				}
			else if ((textfield.value.charAt(i) == 'é') || (textfield.value.charAt(i) == 'è') || (textfield.value.charAt(i) == 'ê') || (textfield.value.charAt(i) == 'à') || (textfield.value.charAt(i) == 'ù') || (textfield.value.charAt(i) == 'ç') || (textfield.value.charAt(i) == 'ô') || (textfield.value.charAt(i) == 'û') || (textfield.value.charAt(i) == 'î'))
				{
				fieldvalue += textfield.value.charAt(i);
				}
			else
...
function checkedformlaunch(formname){
   if (submitfinalcheck(document.forms[formname], "") == true){
      ...
      if (dateErrors.length != 0){
         message += "<?php print_string('dateformaterror', 'block_courseshop') ?>:\n";
         for (var i = 0; i < dateErrors.length; i++){
            message += document.forms[formname].elements[dateErrors[i]+ "_datefault"].value + "\n";
         }
      }
      if (mandatoryErrors.length != 0){
         message += "<?php print_string('mandatoryerror', 'block_courseshop') ?>\n";
