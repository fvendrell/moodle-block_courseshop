...
class shop_handler_PK_LAUREAT{
    ...
    function produce_postpay(&$data){
        ...
        if ($course = get_record('course', 'idnumber', $this->courseidnumber)){
			...
			courseshop_trace("[{$data->transactionid}] Post Pay Process : PK_LAUREAT Failure for course ID number {$this->courseidnumber}.");
    	...
    	courseshop_trace("[{$data->transactionid}] Post Pay Process : PK_LAUREAT Completed.");
        return $productiondata;
    }
