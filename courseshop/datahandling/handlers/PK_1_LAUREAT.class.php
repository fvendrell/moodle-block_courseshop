...
class shop_handler_PK_1_LAUREAT extends shop_handler{
    ...
    function produce_postpay(&$data){
        ...
        foreach($this->courseidnumbers as $courseidnumber){
	        if ($course = get_record('course', 'idnumber', $courseidnumber)){
    			...
    			courseshop_trace("[{$data->transactionid}] Post Pay Process : PK_1_LAUREAT Failure for course ID number $courseidnumber.");
    	...
    	courseshop_trace("[{$data->transactionid}] Post Pay Process : PK_1_LAUREAT Completed.");
        return $productiondata;
    }
