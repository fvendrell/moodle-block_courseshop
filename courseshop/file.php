<?php // $Id: file.php,v 1.1 2011-06-02 17:14:23 vf Exp $

//  Manage all uploaded files in a courseshop file area
...
//  USER is a persistent variable using sessions
    ...
    require($CFG->libdir.'/adminlib.php');
   	...
   	$id      = required_param('id', PARAM_INT); // the courseshop block id
   	$pinned  = required_param('pinned', PARAM_INT); // the pinnedstatus
	...
	$context = get_context_instance(CONTEXT_BLOCK, $id);
  	require_capability('block/courseshop:manage', $context);

    function courseshop_html_footer() {
        global $choose;
    ...
    function courseshop_html_header($wdir, $formfield=""){
        global $CFG, $ME, $choose, $SITE;

        $navlinks = array();
        $navlinks[] = array('name' => get_string('productfiles', 'block_courseshop'), 'link' => '', 'type' => 'title');

        $strfiles = get_string('productfiles', 'block_courseshop');
        ...
        if ($wdir == "/") {
            ...
            $navlinks[] = array('name' => $strfiles,
                                'link' => $ME."?id={$id}&amp;pinned={$pinned}&amp;wdir=/&amp;choose=$choose",
                                'type' => 'misc');

            for ($i = 1; $i < $numdirs-1; $i++) {
                $link .= "/".urlencode($dirs[$i]);
                $navlinks[] = array('name' => $dirs[$i],
                                    'link' => $ME."?id={$id}&amp;pinned={$pinned}&amp;wdir={$link}&amp;choose={$choose}",
                                    'type' => 'misc');
            }
        ...
        if ($choose) {
            ...
            print_header("shortname: $strfiles", $fullname, $navigation,  $formfield);
        }
    ...
    if (! $basedir = make_upload_directory('courseshop/'.$id)) {
    ...
    switch ($action) {
        ...
        case "rename":
            if (($name != '') and confirm_sesskey()) {
                ...
                courseshop_html_header($wdir, 'form.name');
        ...
        case "makedir":
            if (($name != '') and confirm_sesskey()) {
                ...
                } else if (! make_upload_directory("/courseshop/{$id}/{$wdir}/{$name}")) {
        ...
        case "zip":
            if (($name != '') and confirm_sesskey()) {
                ...
                if (setfilelist($_POST)) {
                    ...
                    print_simple_box_start("center");
                    courseshop_printfilelist($USER->filelist);
                    print_simple_box_end();
                    echo "<br />";
            ...
            if (($file != '') and confirm_sesskey()) {
                ...
                if (!$list = $archive->listContent(cleardoubleslashes("{$basedir}{$wdir}"))) {
                    ...
                    foreach ($list as $item) {
                        echo "<tr>";
                        courseshop_print_cell("left", s($item['filename']), 'name');
                        if (! $item['folder']) {
                            courseshop_print_cell("right", display_size($item['size']), 'size');
                        } else {
                            echo "<td>&nbsp;</td>";
                        }
                        $filedate  = userdate($item['mtime'], get_string("strftimedatetime"));
                        courseshop_print_cell("right", $filedate, 'date');
                        echo "</tr>";
                    }
                    echo "</table>";
                }
        ...
        case "restore":
            ...
            if (($file != '') and confirm_sesskey()) {
                ...
                notice_yesno (get_string('areyousuretorestorethis'),
                                $restore_path."?id={$id}&amp;pinned={$pinned}&amp;file=".cleardoubleslashes($wdir."/".$file)."&amp;method=manual",
                                "file.php?id={$id}&amp;pinned={$pinned}&amp;wdir={$wdir}&amp;action=cancel");
            } else {
                courseshop_displaydir($wdir);
            }
            courseshop_html_footer();
...
function courseshop_setfilelist($VARS) {
    global $USER;
...
function courseshop_clearfilelist() {
    global $USER;
...
function courseshop_printfilelist($filelist) {
    global $CFG, $basedir;
    ...
    foreach ($filelist as $file) {
        if (is_dir($basedir.'/'.$file)) {
            ...
            courseshop_printfilelist($subfilelist);
...
function courseshop_print_cell($alignment='center', $text='&nbsp;', $class='') {
    if ($class) {
        $class = ' class="'.$class.'"';
    }
...
function courseshop_displaydir ($wdir) {
//  $wdir == / or /a or /a/b/c/d  etc
    ...
    if (!empty($dirlist)) {
        ...
        foreach ($dirlist as $dir) {
            ...
            if ($dir == '..') {
                ...
                courseshop_print_cell();
                // alt attribute intentionally empty to prevent repetition in screen reader
                courseshop_print_cell('left', '<a href="file.php?id='.$id.'&amp;pinned='.$pinned.'&amp;wdir='.$fileurl.'&amp;choose='.$choose.'"><img src="'.$CFG->pixpath.'/f/parent.gif" class="icon" alt="" />&nbsp;'.get_string('parentfolder').'</a>', 'name');
                courseshop_print_cell();
                courseshop_print_cell();
                courseshop_print_cell();
                ...
                if ($wdir.$dir === '/moddata') {
                    ...
                    courseshop_print_cell('center', "<input type=\"checkbox\" name=\"file$count\" value=\"$fileurl\" />", 'checkbox');
                }
                courseshop_print_cell('left', "<a href=\"file.php?id={$id}&amp;pinned={$pinned}&amp;wdir=$fileurl&amp;choose=$choose\"><img src=\"$CFG->pixpath/f/folder.gif\" class=\"icon\" alt=\"$strfolder\" />&nbsp;".htmlspecialchars($dir)."</a>", 'name');
                courseshop_print_cell('right', $filesize, 'size');
                courseshop_print_cell('right', $filedate, 'date');
                if ($wdir.$dir === '/moddata') {
                    courseshop_print_cell();
                } else { 
                    courseshop_print_cell("right", "<a href=\"file.php?id={$id}&amp;pinned={$pinned}&amp;wdir=$wdir&amp;file=$filesafe&amp;action=rename&amp;choose=$choose\">$strrename</a>", 'commands');
                }
            }
    ...
    if (!empty($filelist)) {
        ...
        foreach ($filelist as $file) {
            ...
            courseshop_print_cell("right", display_size($file_size), 'size');
            courseshop_print_cell("right", $filedate, 'date');
            ...
            if ($icon == "text.gif" || $icon == "html.gif") {
                $edittext .= "<a href=\"file.php?id=$id&amp;pinned={$pinned}&amp;wdir=$wdir&amp;file=$fileurl&amp;action=edit&amp;choose=$choose\">$stredit</a>";
            } else if ($icon == "zip.gif") {
                $edittext .= "<a href=\"file.php?id=$id&amp;pinned={$pinned}&amp;wdir=$wdir&amp;file=$fileurl&amp;action=unzip&amp;sesskey=$USER->sesskey&amp;choose=$choose\">$strunzip</a>&nbsp;";
                $edittext .= "<a href=\"file.php?id=$id&amp;pinned={$pinned}&amp;wdir=$wdir&amp;file=$fileurl&amp;action=listzip&amp;sesskey=$USER->sesskey&amp;choose=$choose\">$strlist</a> ";
                if (!empty($CFG->backup_version) and has_capability('moodle/site:restore', get_context_instance(CONTEXT_COURSE, $id))) {
                    $edittext .= "<a href=\"file.php?id=$id&amp;pinned={$pinned}&amp;wdir=$wdir&amp;file=$filesafe&amp;action=restore&amp;sesskey=$USER->sesskey&amp;choose=$choose\">$strrestore</a> ";
                }
            }

            courseshop_print_cell("right", "$edittext <a href=\"file.php?id={$id}&amp;pinned={$pinned}&amp;wdir=$wdir&amp;file=$filesafe&amp;action=rename&amp;choose={$choose}\">$strrename</a>", 'commands');
